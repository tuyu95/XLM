...
split_data() {
    ...
    echo $NTRAIN
    echo $NVAL
    sort --random-source=<(get_seeded_random 42) $1 | head -$NTRAIN             > $2;
    sort --random-source=<(get_seeded_random 42) $1 | head -$NVAL | tail -5000  > $3;
    sort --random-source=<(get_seeded_random 42) $1 | tail -5000                > $4;
}
for lg in $(echo $pair | sed -e 's/\-/ /g'); do
