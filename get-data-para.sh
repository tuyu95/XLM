...
split_data() {
    ...
    NTRAIN=$((NLINES - 1000));
    NVAL=$((NTRAIN + 500));
    shuf --random-source=<(get_seeded_random ) $1 | head -$NTRAIN             > $2;
    shuf --random-source=<(get_seeded_random ) $1 | head -$NVAL | tail -5000  > $3;
    shuf --random-source=<(get_seeded_random ) $1 | tail -5000                > $4;
}
for lg in $(echo $pair | sed -e 's/\-/ /g'); do
